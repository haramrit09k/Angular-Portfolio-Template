...
The repo root *is* the site — no "Root Directory" / base directory setting
needed on either platform. (`__old_code_to_remove__/` is inert leftover
from a prior version of this repo and can be safely deleted whenever;
Vercel/Netlify's zero-config detection only looks at root-level files, so
its presence doesn't affect deployment.)

### Vercel

- Import the repo. Framework preset: **Other**. Build command: none —
  it's static.
...
- New site from Git. Build command: none — it's static.
...
__old_code_to_remove__/       Prior version of this repo (unrelated Angular
                               app), kept for now, safe to delete anytime.
```
